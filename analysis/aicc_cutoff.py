...
print("""Before running this code, change the file name on line 22!
It should be the .p file generated by the run_analysis.py script.""")

tree_filename = "UPGMA_song_match_2_Shen_to_South.nwk"
picle_filename = "all_analyses_4pulses.p" # Change  this file name!
...
def aicc_diff_from_bm_data(
    ...
    for _ in range(n_datasets):
        ...
        rand_cov_matrix = boat.build_bm_cov_matrix(tree, leaves, rand_var_p, rand_var_bm)
        ...
        for i in range(len(real_mean)):
            relative_meas_error = np.random.uniform(relative_meas_error_min, relative_meas_error_max)
            scale = np.abs((relative_meas_error * real_mean[i]))
            num_artificial_measurements = np.random.randint(n_samples_min, n_samples_max)
            samples = np.random.normal(loc=real_mean[i], scale=scale, size=num_artificial_measurements)
        ...
        bm_analysis = boat_submission.bm_mle(data, tree_copy, leaves, distances, error_variances=error_vars)
        pulse_analyses = (boat_submission
                          .pulse_iterate_configs(
                              data,
                              tree_copy,
                              leaves,
...
def aicc_diff_from_pulse_data(
    ...
    for _ in range(n_datasets):
        ...
        rand_cov_matrix = boat.build_cov_matrix(tree, leaves, rand_pulses, rand_var_p, rand_var_d)
...
labels = ['monte_carlo_params', 'data_filenames', 'bm_generated_aicc_diff', 'pulse_generated_aicc_diff']

all_differences = [labels, mc_parameters, data_filenames, bm_differences, pulse_differences]
